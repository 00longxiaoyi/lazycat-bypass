...
const clashMetaExcludeAppImages = Array.from({ length: 5 }, (_, index) => ({
  src: `/imgs/clash-meta/Android/clash-meta-set-export-app-${index + 1}.png`,
  alt: `Clash Meta 排除应用第 ${index + 1} 步`
}));

type ClashMetaMobileSolutionProps = {
  app: ProxyApp;
  mode: DeviceMode;
  onBack: () => void;
};

function ClashMetaKnownIssues() {
  return (
    <div className="space-y-3">
      <div className="rounded-box border border-base-300 bg-base-100 p-4 text-sm leading-6 text-base-content/80">
        ...
        <ol className="list-decimal space-y-2 pl-5">
          <li>Clash Meta Android 端当前只需要配置排除应用。</li>
          <li>将懒猫微服相关应用加入排除列表，避免本地服务流量被代理接管。</li>
          <li>配置完成后，如仍无法访问懒猫服务，请重新启动 Clash Meta 或重新连接代理。</li>
        </ol>
      </div>
    </div>
  );
...
function ClashMetaExcludeAppPanel() {
  return (
    <div className="rounded-box border border-base-300 bg-base-100 p-4 text-sm leading-6 text-base-content/80">
      <p>将懒猫微服相关应用加入 Clash Meta 的排除应用列表，避免本地服务流量进入代理回环。</p>
      <div className="mt-4 flex justify-end">
        <MobileImageGuideDialog
          title="排除应用"
          ariaLabel="查看 Clash Meta 排除应用图示"
          buttonLabel="如何排除应用"
          images={clashMetaExcludeAppImages}
        />
      </div>
    </div>
  );
...
export function ClashMetaMobileSolution({ app, mode, onBack }: ClashMetaMobileSolutionProps) {
  ...
  const stepsContent = (
    <MobileStepPager
      ...
      renderStepContent={() => <ClashMetaExcludeAppPanel />}
    />
  );
  ...
  return (
    <MobileSolutionFrame
      ...
      tabs={[
        ...
        { key: "known", label: "使用须知", content: <ClashMetaKnownIssues /> }
      ]}
    />
  );
}
